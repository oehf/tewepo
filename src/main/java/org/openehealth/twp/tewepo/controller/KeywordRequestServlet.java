...
/**
 * Servlet implementation for requesting a new keyword.
 * 
 * @author Benjamin Schneider
 * 
 */
@SuppressWarnings("serial")
public class KeywordRequestServlet extends HttpServlet {

	private Logger logger = Logger.getLogger(KeywordRequestServlet.class);//"webportal"
	...
	protected void doPost(HttpServletRequest request,
		...
		if (identity.equals("passwordrequest")) {
			...
			if (username != null && !username.equals("") && surname != null
					&& !surname.equals("") && forename != null
					&& !forename.equals("") && email != null
					&& !email.equals("")) {
				try {

						IPerson requestedPerson = Person.getPerson(username, surname,
								forename, email);

						/*
						 * if the person is present, generate a password and
						 * send this to the contained email
						 */
					...
					logger.error("KeywordRequestServlet - " + e.getMessage());
				...
				logger
						.error("KeywordRequestServlet - Unvollst&auml;ndige Daten");
				request.setAttribute("message", message);
				rd = request.getRequestDispatcher("keywordpage.jsp");
			...
			logger
					.error("KeywordRequestServlet - Es ist ein Fehler aufgetreten");
			request.setAttribute("message", message);
			rd = request.getRequestDispatcher("keywordpage.jsp");
